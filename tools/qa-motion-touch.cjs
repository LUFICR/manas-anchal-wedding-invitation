...
(async () => {
  ...
  const p = await b.newPage({
    viewport: { width, height: width === 430 ? 932 : width === 393 ? 852 : 844 },
    isMobile: true,
    hasTouch: true,
    deviceScaleFactor: 3,
  });
  ...
  for (const id of ['journey','mata','mehendi','haldi','vivah','closing']) {
    ...
    if(await title.count()) assert.notEqual(await title.evaluate(e=>getComputedStyle(e).clipPath),'inset(0px 0px 100%)');
  }
  await p.screenshot({path:`tools/polish-ending-${width}.png`});
  await p.close();
  }
  await b.close();
})().catch((e) => {
